...
Create the following environment variables with the adjustments to your context
...
Create `clusterctl.yaml` from the following script
...
Indicate in the cluster api configuration file `clusterctl.yaml` that you want to use images from the private repo as follow

```yaml
images:
  all:
    repository: localhost:5001/docker.io
```

Install capi following the same make targets as